...
process_all_seasons_fhts <- function(bird_data, fishing_data) {
  # Get unique seasons
...
match_front <- function(grid, front_freq_base_path, seasons) {
  # Initialize an empty list to store the processed data for each season
  season_results <- list()
  
  # 1. Read the front frequency file for the current season
  front_file <- paste0(front_freq_base_path)
  front_df <- read_rds(front_file)
  front_df1 <- front_df %>% 
    group_by(cell, season) %>%
    summarise(
      pfront = mean(pfront, na.rm = TRUE),
      msfront = mean(msfront, na.rm = TRUE),
      .groups = "drop"
    )  # Summarize both metrics  
  
  # Loop through each season
  for (season in seasons) {
    # 2. Filter the grid_ data for the current season
    front_season <- front_df1 %>% filter(season == !!season)
    grid_season <- grid %>% filter(season == !!season)
    
    # 3. Match front frequency with grid for the current season
    matched_data <- merge(grid_season, front_season, by = "cell", all.x = TRUE)
